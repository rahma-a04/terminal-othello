...
let evaluate_move (move : string) : int * int =
  ...
  (row, col)
(*TODO: this impl isn't defensive enough; need to catch invalid argument
  errors*)
...
let rec play_game board (is_black : bool) =
  ...
  | _ ->
      let row_col = evaluate_move response in
      play_game
        (Board.place_piece (fst row_col) (snd row_col) piece board)
        (not is_black)
